...
This is a big one to know; at least 3 problems are based off this on every exam. You can use the acronym PUMA BS CE BLTA to memorize this order (PUMA -> (postfix, unary, multiplicative, additive), BS -> (bit shifts), C -> (comparison), E -> (equality), B -> (bitwise operations), L -> (logic), T -> (ternary), A -> (assignment)).
