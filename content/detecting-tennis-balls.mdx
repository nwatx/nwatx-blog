...
The program is written in Python and uses [OpenCV](https://opencv.org/) to detect tennis balls. Here is
the detection pipeline:

1. Resize the image to 640 x 480 pixels.
2. Run the image through my custom trained version (yes, I did label an entire
   dataset) of the YOLO neural network. Below is a diagram of the model
   architecture
...
4. Run the image through a
   [traveling salesman](https://en.wikipedia.org/wiki/Travelling_salesman_problem)
   optimization algorithm to find the best path to traverse the image.
