...
We can use digit dp to solve this problem. Let $\texttt{dp}[i][j]$ represent the
number of ways to create a length $|K|$ number which is a multiple of $D$ given
that we've already done $i$ digits and a sum equivalent to $j \pmod D$.

To find the answer, we will take a prefix of $K$ and calculate the number of
...
Fortunately, you can do this in
[$\mathcal O(3^N)$](https://cp-algorithms.com/algebra/all-submasks.html#toc-tgt-1),
which runs in time for $N\leq 16$.
...
Let $\texttt{dp}[i]$ represent the number of ways to walk from $(1, 1)$ to the $i$th square without passing any other squares other than the $i$th one.
