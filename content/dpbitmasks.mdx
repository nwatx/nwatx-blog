...
We can check whether or not the $k$-th bit is set by using $S \& (1 \ll (k-1))$. Another equivalent operaton is $(n \gg (k-1))\& 1$. You can read more about how this works [here](https://www.geeksforgeeks.org/check-whether-k-th-bit-set-not/).
...
This works because $1(1\ll n)-1)$ contains exactly $n$ ones (ex. $2^5-1=011111_2$).
...
vi adj[21];
ll dp[1<<21][21]; // amount of flights of subset S ending at city d
