...
Therefore, iterating over the $2^n$ number subsets is much faster than iterating through the $n!$ number of permutations. [CPH](/CPH.pdf) further notes that for an input of size $n=20$, then $n! \approx 2.4\cdot 10^{18}$ which greatly exceeds $2^{20}\approx 10^6$.
