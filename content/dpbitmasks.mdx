...
Then, for each vertex, we check if $v$ is in the mask with $s \& (1<<(v-1))$.

If $v$ is in the mask, then we can add the total amount of flights to the subset $S$.

We can finally query the flights that travel through the cities $\{1,2,\ldots,n\}$ with $\texttt{dp}[(1<<n)-1][n]$.
